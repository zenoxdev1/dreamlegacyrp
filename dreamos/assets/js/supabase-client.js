...
function api(path, method, body) {
    ...
    if (path === "/api/gram/post") {
        return sb.rpc("dlrp_gram_create_post", { p_token: body.key, p_body: body.body || null, p_image_data: body.imageData || null }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return r.data;
        });
    }

    if (path === "/api/gram/feed") {
        return sb.rpc("dlrp_gram_list_feed", { p_token: body.key }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return { posts: r.data };
        });
    }

    if (path === "/api/gram/like") {
        return sb.rpc("dlrp_gram_toggle_like", { p_token: body.key, p_post_id: body.postId }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return r.data;
        });
    }

    if (path === "/api/gram/delete") {
        return sb.rpc("dlrp_gram_delete_post", { p_token: body.key, p_post_id: body.postId }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return r.data;
        });
    }

    if (path === "/api/gram/dm/send") {
        return sb.rpc("dlrp_gram_send_dm", { p_token: body.key, p_to_rp_name: body.toRpName, p_body: body.body }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return r.data;
        });
    }

    if (path === "/api/gram/dm/thread") {
        return sb.rpc("dlrp_gram_get_dm_thread", { p_token: body.key, p_other_rp_name: body.otherRpName }).then(function (r) {
            if (r.error) rpcFail(r.error);
            return { messages: r.data };
        });
    }
